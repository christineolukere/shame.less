...
DO $$
BEGIN
  -- Index for journal_media entry_id
  IF NOT EXISTS (
  ...
  -- Index for journal_media user_id
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes 
  ...
  -- Index for journal_entries title
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes 
  ...
  -- Index for journal_entries mood_rating
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes 
