...
const Navigation: React.FC<NavigationProps> = ({ currentView, onNavigate }) => {
  ...
  return (
    <nav className="fixed bottom-0 left-1/2 transform -translate-x-1/2 w-full max-w-md bg-white/95 backdrop-blur-sm border-t border-sage-100 safe-bottom">
      <div className="flex items-center justify-around py-1 sm:py-2 mobile-container">
        {navItems.map((item) => {
          ...
          return (
            <motion.button
              ...
              className={`flex flex-col items-center p-2 rounded-lg transition-colors touch-target ${
                isActive 
              ...
              whileTap={{ scale: 0.95 }}
            >
              <Icon className={`w-4 h-4 sm:w-5 sm:h-5 ${isActive ? 'fill-current' : ''}`} />
